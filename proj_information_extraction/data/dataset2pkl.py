# -*-coding:utf-8 -*-
"""
data processing for bilstm_crf_model model
"""
import re
import pickle
import random
import numpy as np
from bilstm_crf_model.bilstm_crf import Config

msra_dpath = "../dataset/MSRA/"
train_fname = "train1.txt"
word2tag_fname = "word2tag_bilstm.txt"
...
# tag to id
with open('word2tag_bert.txt', 'r') as f:
    for line in f:
...
tag2id[config.start_tag] = len(tag2id)
tag2id[config.end_tag] = len(tag2id)
id2tag[len(id2tag)] = config.start_tag
id2tag[len(id2tag)] = config.end_tag


def sentence_padding(sentence):
    """把 sentences 转为 word_ids 形式，并自动补全位 config.max_len 长度。"""
...
def tag_padding(tags):
    """把 tags 转为 id 形式， 并自动补全位 config.max_len 长度。"""
    if len(tags) >= config.max_len:  # 长则弃掉
        return tags[:config.max_len]
    tags.extend([0] * (config.max_len - len(tags)))  # 短则补全
    return tags
